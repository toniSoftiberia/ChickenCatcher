...
public class ChickenMovement : MonoBehaviour {
    ...
    public bool CheckForward() {
        ...
        if (Physics.Raycast(rayLeft, out hit, seekDistance, collisionMask)) {
        ...
        if (Physics.Raycast(rayCenterLeft, out hit, seekDistance, collisionMask)) {
            if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Player")) {
                Debug.Log("Player Left");
                playerFront = 1;
            } else {
        ...
        if (Physics.Raycast(rayRight, out hit, seekDistance, collisionMask)) {
            if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Player")) {
        ...
        if (Physics.Raycast(rayCenterRight, out hit, seekDistance, collisionMask)) {
            if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Player")) {
                Debug.Log("Player Right");
                playerFront = 1;
            } else {
        ...
        if (Physics.Raycast(ray, out hit, seekDistance, collisionMask)) {
            if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Player")) {
